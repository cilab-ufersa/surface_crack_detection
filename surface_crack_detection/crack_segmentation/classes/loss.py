...
class Loss:
    ...
    def define_Loss(self):
        ...
        if self.args['loss'] == 'Focal_Loss':
            from subroutines.loss_metrics.loss_metrics import Focal_Loss
            loss = Focal_Loss(
                alpha=self.args['focal_loss_a'], gamma=self.args['focal_loss_g'])

        elif self.args['loss'] == 'WCE':
            from subroutines.loss_metrics import Weighted_Cross_Entropy
            loss = Weighted_Cross_Entropy(beta=self.args['WCE_beta'])

        elif self.args['loss'] == 'F1_score_Loss':
            from subroutines.loss_metrics import F1_score_Loss
            loss = F1_score_Loss

        elif self.args['loss'] == 'F1_score_Loss_dil':
            from subroutines.loss_metrics import F1_score_Loss_dil
