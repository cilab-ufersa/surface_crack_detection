...
def Visualize_Predictions(args, predictions, threshold=0.5):
    ...
    from subroutines.loss_metrics import compute_iou
    ...
    for ii in range(0, numImages):
        ...
        iou_score = compute_iou(db["labels"][ii].squeeze(), predictions[ii])
        ...
        plt_subtitle = "F1:{0}% / RE:{1}%\nPR:{2}% / IoU: {3}".format(
            f1_score, recall, precision, iou_score
        )
        ...
        image_bin = "{}{}.png".format('surface_crack_detection/crack_segmentation/output/masks/',ii)

        # Set title for prediction
        ax3.set_title(plt_subtitle, fontsize=7)
        
        cv2.imwrite(image_bin, (prediction*255.0).astype("uint8"))
