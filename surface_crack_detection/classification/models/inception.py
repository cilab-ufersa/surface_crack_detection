...
train_df, test_df = train_test_split(
    dataset.sample(frac=1.0), train_size=0.80, random_state=42)
...
history = model.fit(train_data, validation_data=valid_data,
                    epochs=14, verbose=1, callbacks=[callbacks])

history = history.history

# pickle the history to file
with open('surface_crack_detection/classification/models/historys/inception_model_history', 'wb',) as f:
    pickle.dump(history, f)

# saving the model to inception_model.h5 file
model.save('surface_crack_detection/classification/models/trained/inception_model.h5')
...
# saving the curves
plt.savefig('surface_crack_detection/classification/models/figures/inception_curves.jpg')
...
plt.savefig('surface_crack_detection/classification/models/figures/inception_matrix_confusion.jpg')